...
mod:SetRevision(("$Revision: 6625 $"):sub(12, -3))
...
local timerCrushArmor	= mod:NewStackTimer(20, 103687, nil, mod:IsTank())
...
local timerVortex	= mod:NewDurationTimer(5, 110047)
local timerVortexNext	= mod:NewNextTimer(97, 110047)
local timerBlood	= mod:NewDurationTimer(19, 103851)
...
function mod:SPELL_CAST_SUCCESS(args)
	if args:IsSpellID(103640) and GetTime() - spamCrystal > 3 then
		...
		timerCrystal:Start()
		spamCrystal = GetTime()
	elseif args:IsSpellID(110047) then
		warnVortex:Show()
		timerVortex:Start()
...
function mod:SPELL_DAMAGE(args)
	if args:IsSpellID(108570) and GetTime - spamBlood > 3 then
		specwarnBlood:Show()
		spamBlood = GetTime()
	end
end