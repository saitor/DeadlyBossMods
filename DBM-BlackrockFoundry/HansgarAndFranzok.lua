...
mod:SetRevision(("$Revision: 13061 $"):sub(12, -3))
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	...
	elseif spellId == 157922 and DBM.Options.DebugMode then--First jump must use 157922
		...
		if not self.vb.firstJump then
			...
			self:BossTargetScanner(cachedGUID, "JumpTarget", 0.05, 40, nil, nil, false)--Don't include tank in first scan should be enough of a filter for first, it'll grab whatever first non tank target he gets and set that as first jump target and it will be valid
		else--Not first jump
			DBM:Debug("157922: firstJump false")
			self:BossTargetScanner(cachedGUID, "JumpTarget", 0.05, 40, nil, nil, true, nil, self.vb.lastJumpTarget)--1.3 seconds worth of scans, because i've seen it take as long as 1.2 to get target, and yet, still faster than 157923 by 0.6 seconds. Most often, it finds target in 0.5 or less
		end
