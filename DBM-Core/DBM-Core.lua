...
-------------------------------
DBM = {
	Revision = tonumber(("$Revision: 13788 $"):sub(12, -3)),
	...
	function specialWarningPrototype:Show(...)
		if not DBM.Options.DontShowSpecialWarnings and (not self.option or self.mod.Options[self.option]) and not moving and frame then
			...
			if self.sound then
				...
				if type(soundId) == "number" then soundId = noteHasName end--Change number to 5 if it's not a custom sound, else, do nothing with it
