...
-------------------------------
DBM = {
	Revision = tonumber(("$Revision: 12217 $"):sub(12, -3)),
...
--			end
			...
			if IsInGroup() then
				self:Schedule(6, self.RequestTimers, self)--Break timer recovery doesn't work if outside the zone when reloadui or relogging (no loadmod). Need request timer here.
			end
		end
	end
end
...
do
	...
	function DBM:UpdateSpecialWarningOptions()
		...
		font:SetFont(self.Options.SpecialWarningFont, self.Options.SpecialWarningFontSize, "THICKOUTLINE")
		font:SetTextColor(unpack(self.Options.SpecialWarningFontCol))
	end
