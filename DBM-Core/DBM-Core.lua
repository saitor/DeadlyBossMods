...
-------------------------------
DBM = {
	Revision = tonumber(("$Revision: 11905 $"):sub(12, -3)),
...
bossModPrototype.PlayVoice = DBM.PlayVoice
...
do
	...
	function DBM:PlayVoice(mod, spellId, type)
		if DBM.Options.ChosenVoicePack == "None" then return end
		local voice = DBM.Options.ChosenVoicePack
		local path = "Interface\\AddOns\\DBM-VP"..voice.."\\"..mod.instanceId.."\\"..mod.id.."\\"..spellId..type..".ogg"
		--Example "Interface\\AddOns\\DBM-VPHenry\\1228\\1128\\158986now.ogg" --This would be Kargath playvoice for berserker rush from highmaul raid generated by self:PlayVoice(self, spellId, "now")
		--types: "now", "soon", "in5" --Other types as needed, mainly so we can use multiple voies for same spellid if needed (to say, have both a soon and now warning)
		--If no file at path, it should silenty fail. However, I want to try to only add PlayVoice to mods for files that already exist.
		if DBM.Options.UseMasterVolume then
