...
-------------------------------
DBM = {
	Revision = tonumber(("$Revision: 12145 $"):sub(12, -3)),
...
do
	...
	function specialWarningPrototype:Show(...)
		if DBM.Options.ShowSpecialWarnings and (not self.option or self.mod.Options[self.option]) and not moving and frame then
			...
			if self.sound and self.mod.Options[self.option .. "SpecialWarningSound"] ~= "None" then
				if not self.hasVoice or not DBM.Options.VoiceOverSW or not self.mod.Options["Voice" .. self.spellId .. (self.voiceOptionVersion or "")] or ((self.voiceVersion or 0) > DBM.VoicesVersion) then
	...
	function bossModPrototype:NewSpecialWarning(text, optionDefault, optionName, noSound, runSound, hasVoice, voiceVersion, voiceOptionVersion)
		if not text then
		...
		local obj = setmetatable(
			{
				...
				voiceOptionVersion = voiceOptionVersion,
			},
			mt
		)
	...
	local function newSpecialWarning(self, announceType, spellId, stacks, optionDefault, optionName, noSound, runSound, optionVersion, hasVoice, voiceVersion, voiceOptionVersion)
		if not spellId then
	...
	function bossModPrototype:NewSpecialWarningStack(text, optionDefault, stacks, optionName, noSound, runSound, optionVersion, hasVoice, voiceVersion, voiceOptionVersion)
		...
		return newSpecialWarning(self, "stack", text, stacks, optionDefault, optionName, noSound, runSound, optionVersion, hasVoice, voiceVersion, voiceOptionVersion)
	end
	...
	function bossModPrototype:NewSpecialWarningPreWarn(text, optionDefault, time, optionName, noSound, runSound, optionVersion, hasVoice, voiceVersion, voiceOptionVersion)
		if type(text) == "string" and text:match("OptionVersion") then
			local temp = optionVersion
			optionVersion = string.sub(text, 14)
			text, optionDefault, time, optionName, noSound, runSound = optionDefault, time, optionName, noSound, runSound, temp
		end
		return newSpecialWarning(self, "prewarn", text, time, optionDefault, optionName, noSound, runSound, optionVersion, hasVoice, voiceVersion, voiceOptionVersion)
	end
