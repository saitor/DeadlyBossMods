...
mod:SetRevision(("$Revision: 8136 $"):sub(12, -3))
...
function mod:SPELL_CAST_START(args)
	if args:IsSpellID(122398) then
		...
		if args:GetSrcCreatureID() == 62701 then--Cast by a wild construct not controlled by player
			...
			DBM:AddMsg("Debug: Wild Contruct casting Explosion", GetTime(), lastStrike or 0)--Lets see what get time return and what last strike returns
			if playerIsConstruct and GetTime() - lastStrike >= 4 then--Player is construct and Amber Strike will be available before cast ends.
				DBM:AddMsg("Debug: You're a construct with available interrupt.")--First logic check passed, this debug tells us we're good so far.
				...
				if self:LatencyCheck() then--if you're too laggy we don't want you telling us you can interrupt it 2-3 seconds from now. we only care if you can interrupt it NOW
					DBM:AddMsg("Debug: You successfully sent sync to raid about that interrupt")--Latency check passed, this debug tells us we should have at least sent a sync now
	...
	elseif args:IsSpellID(122402) then--Amber Monstrosity
		--This doesn't work, for no logical reason what so ever.
		if playerIsConstruct and GetTime() - lastStrike >= 4 then--Player is construct and Amber Strike will be available before cast ends.
			DBM:AddMsg("Debug: You're a construct with available interrupt.")
			...
			if self:LatencyCheck() then--if you're too laggy we don't want you telling us you can interrupt it 2-3 seconds from now. we only care if you can interrupt it NOW
				DBM:AddMsg("Debug: You successfully sent sync to raid about that interrupt")
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args:IsSpellID(122389) and args.sourceGUID == UnitGUID("player") then--Amber Strike
		lastStrike = GetTime()
		DBM:AddMsg("Debug: You just used Amber Strike", lastStrike, args.sourceGUID, UnitGUID("player"))--Maybe GetTime() is messing up, so let see what it is. Also see if for some reason sourceguid doesn't match players GUID
	end
end
...
--	DBM:AddMsg("Debug: DBM Sync", msg, str)--Pretty sure this part is already working so leaving commented out.
...
--		DBM:AddMsg("Debug: String Split Successful", spellId, guid)--Pretty sure this part is already working so leaving commented out.
	end
	if msg == "InterruptAvailable" and guids[guid] and spellId then
		DBM:AddMsg("Debug: InterruptAvailable sync recieve successful")--We got the sync, so now we know we got this far.
